...
class UserForm(forms.Form):
    ...
    def clean_name(self):
        # Плохой пример - подмена параметра
        name = self.cleaned_data['name']
        if len(name) < 3:
            raise forms.ValidationError('Имя должно состоять не менее чем из 3 символов')
        return name

    def clean_email(self):
        email = self.cleaned_data['email']
        if not (email.endswith('vk.team') or email.endswith('corp.mail')):
            raise forms.ValidationError('Укажите корпоративную почту!')
        return email
